...
export default function Home() {
  const cardsArray = [
    { heading: "Shares", icon: faProjectDiagram, count: 18, completed: 2 },
    { heading: "Activity", icon: faTasks, count: 132, completed: 5 },
    { heading: "Communication", icon: faPeopleArrows, count: 12, completed: 10 },
    { heading: "Projects", icon: faLayerGroup, count: "76%", completed: 8 },
  ];
  ...
  function extractData() {
    // Move the key to an env variable (see notes), e.g. process.env.REACT_APP_YT_KEY
    fetch(
        "https://youtube.googleapis.com/youtube/v3/playlists?part=snippet%2CcontentDetails&channelId=UCuT0rj__qEq_ZO3kYwun4Qg&maxResults=50&key=AIzaSyAjDMJv0A_TjAaO3A1vtettwmL_BdncsWA"
      )
      .then((response) => response.json())
  ...
  return (
    <div className="home-inner">
      <div className="banner">
        <div className="name">
          Bin Muhammad
          <div className="name-sub">Learning Management System</div>
        </div>
      ...
      <div className="responsive-home">
        {playList.map((elem) => (
          <div className="course-wrap" key={elem.id}>
            <div
              className="course"
              onClick={() => navigate(`/playListItem/${elem.id}`)}
            >
              <img
                ...
                alt={elem.snippet.localized.title}
                loading="lazy"
              />
              <h4 className="course-title">{elem.snippet.localized.title}</h4>
              <p className="course-count">{elem.contentDetails.itemCount}</p>
              <Link className="course-link" to={"/playListItem"}>
                view more Details
              </Link>
            </div>
          </div>
